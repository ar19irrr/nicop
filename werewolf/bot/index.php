...
// ============================================================
// 3. دیتابیس‌ها (فیکس کامل ارور loadGames/saveGames)
// ============================================================
...
// ============================================================
// بقیه توابع (isAdmin, killPlayer, سیستم سکه، درجه‌بندی، گزارش، تنظیمات گروه، امتیاز)
// تماماً همان کد قبلی که تا حالا ارور نداشتند
...
function getShopItems() {
    return [
        ...
        ['id' => 'free_coin', 'name' => '🪙 ۱۰۰ سکنه رایگان', 'price' => 0]
    ];
}
...
function buyItem($user_id, $item_id) {
    ...
    if ($item['price'] == 0) {
        addCoin($user_id, 100);
        return ['success' => true, 'message' => "✅ ۱۰۰ سکنه رایگان دریافت شد!"];
    }
    if (!removeCoin($user_id, $item['price'])) {
        return ['success' => false, 'message' => '❌ سکنه کافی نیست!'];
    }
    return ['success' => true, 'message' => "✅ {$item['name']} خریداری شد!"];
}

// ============================================================
// سیستم درجه‌بندی و سایر توابع
// ============================================================
...
function getRankName($rank) {
    $names = [1 => 'نوب پلیر 😣', 2 => 'پلیر 😕', 3 => 'روستایی 👨🏻', 4 => 'روستایی پررو 😌', 5 => 'دلبر روستا 👰🏻', 6 => 'کلانتر روستا 👮🏻‍♂️', 7 => 'پیشگو روستا 👳🏻‍♂️', 8 => 'فرشته روستا 👼🏻', 9 => 'شکارچی روستا 💂🏻‍♂️', 10 => 'پررو پلیر روستا 👽', 11 => 'فرقه‌گرای نوب 🤮', 12 => 'فرقه‌گرا 👤', 13 => 'رئیس فرقه 🎩', 14 => 'پررو پلیر فرقه 🗣', 15 => 'ومپایر نوب 👩‍🦲', 16 => 'ومپایر 🧛🏻‍♂️', 17 => 'ومپایر اصیل 🧛🏻‍♀️', 18 => 'گرگینه 🐺', 19 => 'گرگ ایکس 🌕🐺', 20 => 'گرگ توله 🐶', 21 => 'گرگ آلفا ⚡️🐺', 22 => 'ملکه جنگل 🧝🏻‍♀️🐺', 23 => 'قاتل زنجیره‌ای 🔪', 24 => 'قاتل سریالی ⚰️', 25 => 'کماندار جنگل 🏹', 26 => 'شوالیه پررو 🎠', 27 => 'اونیکس پلیر 🥉', 28 => 'پایه بالای اونیکس 🥈', 29 => 'پررو پلیر اونیکس 🥇', 30 => 'پادشاه اونیکس 👑'];
...
function addXPAfterGame($game) {
    ...
    foreach ($alive as $p) {
        ...
        if ($result['ranked_up']) sendRankUpMessage($p['id'], $result['old_rank'], $result['new_rank'], $p['name']);
    }
    foreach ($dead as $p) {
...
function sendRankUpMessage($user_id, $old_rank, $new_rank, $user_name = 'کاربر عزیز') {
    ...
    $msg = "☀️☀️بهت کلی تبریک میگم میدونی چرا؟\nچون ارتقا درجه پیدا کردی 🎖\nبه دستور <b>{$user_name}</b> 👑 تو از درجه <b>{$old_name}</b> به درجه <b>{$new_name}</b> ارتقا پیدا کردی.🏅\nحالا برو جلوی دوستات پز بده 👬";
...
function addReport($reporter_id, $reported_id, $reason, $game_code = null) {
    $reports = loadReports();
    $reports[] = ['id' => count($reports) + 1, 'reporter_id' => $reporter_id, 'reported_id' => $reported_id, 'reason' => $reason, 'game_code' => $game_code, 'status' => 'pending', 'created' => time()];
    saveReports($reports);
    return true;
}
...
function getGroupLink($chat_id) {
    $links = loadLinks();
    if (isset($links[$chat_id])) return ['success' => true, 'link' => $links[$chat_id]['link']];
    return ['success' => false, 'message' => '❌ لینکی برای این گروه تنظیم نشده است!'];
}

function setGroupLink($chat_id, $user_id, $link) {
    if (!preg_match('/^https?:\/\/[^\s]+$/', $link)) return ['success' => false, 'message' => '❌ لینک نامعتبر است!'];
    $links = loadLinks();
...
// ============================================================
// 9. توابع اصلی بازی (با فیکس ارور 204 و 504)
// ============================================================
...
function createGame($group_id, $creator_id, $creator_name, $mode = 'normal') {
    ...
    $modes = ['normal' => 'عادی', 'easy' => 'آسان 🎈', 'mafia' => 'مافیا 🐺', 'vampire' => 'ومپایری 🧛‍♂️', 'werewolf' => 'ورولف 🐺', 'bomber' => 'بمب‌گذار 💣', 'foolish' => 'احمقانه 🃏', 'mighty' => 'قدرتی ♨️', 'romantic' => 'عاشقانه 👨‍❤️‍👨', 'coin' => 'سکنه‌ای 💰'];
    $mode_name = $modes[$mode] ?? 'عادی';
    $games[$code] = ['code' => $code, 'group_id' => $group_id, 'creator_id' => $creator_id, 'creator_name' => $creator_name, 'mode' => $mode, 'mode_name' => $mode_name, 'players' => [], 'status' => 'waiting', 'created' => time(), 'wait_until' => time() + 300, 'extend_count' => 0, 'phase' => null, 'night_count' => 0, 'day_count' => 0, 'night_actions' => [], 'votes' => [], 'night_end_time' => 0, 'day_end_time' => 0, 'vote_end_time' => 0, 'day_duration' => 60, 'night_duration' => 60, 'vote_duration' => 60, 'afk_counts' => []];
    saveGames($games);
    $remaining = 300;
    $minutes = floor($remaining / 60);
    $seconds = $remaining % 60;
    $msg = "🎮 <b>بازی {$mode_name} ساخته شد!</b>\n\n🎲 کد: <code>$code</code>\n👤 سازنده: $creator_name\n👥 تعداد: ۰ نفر\n\n⏱ زمان باقیمانده: $minutes:" . sprintf("%02d", $seconds) . "\n\n👇 برای ورود به بازی روی دکمه زیر کلیک کن:";
    $keyboard = [['text' => '🎯 ورود به روستا', 'callback_data' => 'join_' . $code], ['text' => '⚡ شروع زودهنگام (فقط ادمین)', 'callback_data' => 'force_start_' . $code]];
    sendMessage($group_id, $msg, ['inline_keyboard' => [$keyboard]]);
...
function joinGame($code, $user_id, $user_name) {
    ...
    $game = $games[$code] ?? null;
    if (!$game || $game['status'] != 'waiting') {
        return ['success' => false, 'message' => '⏳ این بازی قبلاً شروع شده یا یافت نشد!'];
    }
...
function extendWaitingTime($chat_id, $user_id) {
    $games = loadGames();
    if (!isAdmin($user_id, $chat_id)) return ['success' => false, 'message' => '❌ فقط ادمین‌های گروه می‌توانند زمان را تمدید کنند!'];
    foreach ($games as $code => $game) {
        if (isset($game['group_id']) && $game['group_id'] == $chat_id && $game['status'] == 'waiting') {
            $game['wait_until'] += 30;
            $games[$code] = $game;
...
function setGameTiming($chat_id, $user_id, $timing) {
    ...
    foreach ($games as $code => $game) {
        if ($game['group_id'] == $chat_id && $game['status'] == 'waiting') {
            ...
            if (!isset($times[$timing])) return ['success' => false, 'message' => '❌ گزینه نامعتبر!'];
...
function forceStartGame($group_id, $user_id) {
    ...
    if (!$game) return ['success' => false, 'message' => '❌ بازی فعالی برای شروع وجود ندارد!'];
    if (!isAdmin($user_id, $group_id)) return ['success' => false, 'message' => '❌ فقط ادمین‌های گروه می‌توانند زودتر شروع کنند!'];
    ...
    saveGames($games);
    foreach ($game['players'] as $p) {
        if (!isset($p['role']) || empty($p['role'])) continue;
...
function getRoleActionDescription($role) {
    $actions = ['werewolf' => '🐺 یک نفر را برای خوردن انتخاب کن.', 'alpha_wolf' => '⚡️🐺 یک نفر را برای حمله انتخاب کن.', 'wolf_cub' => '🐶 یک نفر را برای خوردن انتخاب کن.', 'lycan' => '🌝🐺 یک نفر را برای خوردن انتخاب کن.', 'forest_queen' => '🧝🏻‍♀️🐺 یک نفر را برای خوردن انتخاب کن.', 'white_wolf' => '🌩🐺 یک نفر را برای محافظت انتخاب کن.', 'beta_wolf' => '💤🐺 یک نفر را برای خوردن انتخاب کن.', 'ice_wolf' => '☃️🐺 یک نفر را برای منجمد کردن انتخاب کن.', 'enchanter' => '🧙🏻‍♂️ یک نفر را برای طلسم کردن انتخاب کن.', 'honey' => '🧙🏻‍♀️ یک نفر را برای تغییر نقش انتخاب کن.', 'sorcerer' => '🔮 یک نفر را برای دیدن نقش انتخاب کن.', 'vampire' => '🧛🏻‍♂️ یک نفر را برای حمله انتخاب کن.', 'bloodthirsty' => '🧛🏻‍♀️ شما زندانی هستید! ومپایرها باید شما رو آزاد کنن.', 'kent_vampire' => '💍🧛🏻 یک نفر را برای زیر نظر گرفتن انتخاب کن.', 'chiang' => '👩‍🦳 یک نفر را برای بررسی انتخاب کن.', 'serial_killer' => '🔪 یک نفر را برای کشتن انتخاب کن.', 'archer' => '🏹 یک نفر را برای تیراندازی انتخاب کن.', 'davina' => '🍾 یک روز را سکوت کن.', 'seer' => '👁️ یک نفر را برای دیدن نقش انتخاب کن.', 'guardian_angel' => '🛡️ یک نفر را برای محافظت انتخاب کن.', 'knight' => '🗡 یک نفر را برای محافظت انتخاب کن.', 'hunter' => '🔫 کلانتر هستید! ومپایر اصیل در زندان شماست.', 'harlot' => '💋 یک نفر را برای ملاقات انتخاب کن.', 'detective' => '🔍 یک نفر را برای تحقیق انتخاب کن.', 'cupid' => '💘 دو نفر را برای عاشق کردن انتخاب کن.', 'phoenix' => '🪶 شب ۳ و ۵ می‌تونی اشک بدی (۲ تا).', 'sandman' => '💤 همه را بخوابان.', 'spy' => '🦹🏻‍♂️ یک نفر را برای جاسوسی انتخاب کن.', 'cultist' => '👤 یک نفر را برای دعوت به فرقه انتخاب کن.', 'royce' => '🎩 یک نفر را برای دعوت به فرقه انتخاب کن.', 'frankenstein' => '🧟‍♂️🪖 یک نفر را برای محافظت انتخاب کن.', 'monk_black' => '🦇 یک نفر را برای دعوت به فرقه انتخاب کن.', 'fire_king' => '🔥🤴🏻 شب اول نفت بپاش، شب دوم آتش بزن.', 'ice_queen' => '❄️👸🏻 یک نفر را برای منجمد کردن انتخاب کن.', 'lilith' => '🐍👩🏻‍🦳 یک نفر را برای جستجو انتخاب کن.', 'magento' => '🧲 یک نفر را برای جذب انتخاب کن.', 'black_knight' => '🥷🗡 یک نفر را برای دفاع انتخاب کن.', 'bride_dead' => '👰‍♀☠️ یک نفر را برای کشتن انتخاب کن.', 'joker' => '🤡 یک نفر را برای جستجو انتخاب کن.', 'harly' => '👩🏻‍🎤 از جوکر محافظت کن.', 'dian' => '🧞‍♂️ هدف خود را انتخاب کن.', 'dinamit' => '🧨 یک خونه را برای جستجو انتخاب کن.', 'bomber' => '💣 یک خونه را برای بمب‌گذاری انتخاب کن.', 'tso' => '⚔️ یک نفر را برای جستجوی جومونگ انتخاب کن.', 'lucifer' => '😈 یک نفر را برای گول زدن انتخاب کن.'];
...
function sendNightPanel($player, $game) {
    ...
    $msg = "🌙 <b>شب " . $night_count . "</b>\n\n🎭 نقش شما: " . getRoleDisplayName($role) . "\n\n" . getRoleActionDescription($role) . "\n\n👇 یک نفر رو انتخاب کن:";
    $keyboard = [];
    if ($role == 'fire_king') {
        ...
        if (!empty($oiled_houses)) $keyboard[] = [['text' => '💥 آتش زدن همه خونه‌های نفتی', 'callback_data' => 'night_fireking_detonate']];
...
// ============================================================
// 11. پردازش شب و روز (فیکس ارور 570 و 610)
// ============================================================
    ...
    foreach ($games as $code => $game) {
        ...
        if ($game['phase'] == 'night' && isset($game['night_end_time']) && $now >= $game['night_end_time']) {
            ...
            foreach ($game['players'] as $p) {
                ...
                $has_action = false;
                foreach ($night_actions as $action) {
...
if (isset($update['callback_query'])) {
    ...
    if (strpos($data, 'join_') === 0) {
        ...
        $game = $games[$code] ?? null;
        if (!$game || $game['status'] != 'waiting') {
            answerCallbackQuery($callback_id, "⏳ این بازی قبلاً شروع شده یا یافت نشد!", true);
            http_response_code(200);
            echo '{"ok":true}';
            exit;
        }
    ...
    if (strpos($data, 'night_') === 0) {
        ...
        if ($action == 'skip' || $action == 'skip_fireking') {
            ...
            $games[$game_code] = $game;
            saveGames($games);
            answerCallbackQuery($callback_id, "⏭️ این شب رو رد کردید! ✅ ثبت شد");
        } 
        elseif ($action == 'fireking_oil') {
            $game['night_actions'][] = ['player' => $user_id, 'role' => 'fireking_oil', 'target' => (int)$target];
            $games[$game_code] = $game;
            saveGames($games);
            answerCallbackQuery($callback_id, "🛢️ نفت پاشی شد! ✅ ثبت شد");
        } 
        elseif ($action == 'fireking_detonate') {
            $game['night_actions'][] = ['player' => $user_id, 'role' => 'fireking_detonate', 'target' => 'all'];
            $games[$game_code] = $game;
            saveGames($games);
            answerCallbackQuery($callback_id, "💥 آتش زدن تمام خونه‌ها! ✅ ثبت شد");
        } 
        elseif (!empty($target)) {
            $game['night_actions'][] = ['player' => $user_id, 'role' => $role, 'target' => (int)$target];
            $games[$game_code] = $game;
            saveGames($games);
            answerCallbackQuery($callback_id, "✅ انتخاب شما ثبت شد! ✅ ثبت شد");
        }
        $games = loadGames();
        $game = $games[$game_code] ?? null;
        if ($game && $game['phase'] == 'night' && count($game['night_actions']) >= count(getAlivePlayers($game))) {
            processNight($game_code, $game);
        }
        http_response_code(200);
        echo '{"ok":true}';
        exit;
    ...
    if (strpos($data, 'vote_') === 0) {
        ...
        $games[$game_code] = $game;
        saveGames($games);
        $target_name = ($target == 'skip') ? 'سفید' : (getPlayerById($game, $target)['name'] ?? 'نامشخص');
...
switch ($command) {
    case '/start':
        $keyboard = [
            ...
            ['text' => '▶️ شروع عادی', 'callback_data' => 'start_normal'],
            ['text' => '💪 شروع قدرتی', 'callback_data' => 'start_mighty'],
            ['text' => '🧛 شروع ومپایری', 'callback_data' => 'start_vampire'],
            ['text' => '🐺 شروع ورولفی', 'callback_data' => 'start_werewolf']
        ];
        sendMessage($chat_id, "👋 سلام <b>$first_name</b>!\n🐺 به ربات گرگینه خوش اومدی!", ['inline_keyboard' => $keyboard]);
        break;
    ...
    case '/myrank':
        ...
        $msg = "📊 <b>وضعیت درجه شما</b>\n\n🎖️ درجه: <b>{$info['rank']}</b> - {$info['rank_name']}\n📈 XP: <b>{$info['xp']}</b> / <b>{$info['next_xp']}</b>\n📊 پیشرفت: " . round(($info['xp'] / $info['next_xp']) * 100, 1) . "%\n\n💡 هر بازی زنده موندن: +50 XP\n💀 هر بازی مردن: +20 XP";
    ...
    case '/shop':
        ...
        foreach ($items as $item) {
            $price_text = $item['price'] == 0 ? 'رایگان' : $item['price'] . ' سکنه';
...
function sendPlayerList($chat_id, $game) {
    ...
    if ($game['status'] == 'waiting' && isset($game['wait_until'])) {
        ...
        $keyboard = ['inline_keyboard' => [[['text' => '🎯 ورود به روستا', 'callback_data' => 'join_' . $game['code']]]]];
