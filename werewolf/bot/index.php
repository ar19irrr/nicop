<?php
// index.php - نسخه نهایی و کامل (رفع باگ‌های رأی‌گیری و Flood)
...
// ============================================================
// 3. سیستم‌های سکه، رتبه‌بندی، گزارش و تنظیمات (کامل)
// ============================================================
...
function saveGroupSettings($settings) {
    global $data_path;
    if (!is_dir($data_path)) mkdir($data_path, 0777, true);
    file_put_contents($data_path . 'group_settings.json', json_encode($settings, JSON_PRETTY_PRINT));
}
...
// ============================================================
// 5. توابع اصلی بازی (با اصلاحات درخواستی)
// ============================================================
...
function createGame($group_id, $creator_id, $creator_name, $mode = 'normal') {
    ...
    // تغییر: سازنده بازی دیگر خودکار جوین نمی‌شود
    $games[$code] = [
...
function joinGame($code, $user_id, $user_name) {
    ...
    // ارسال لیست بازیکنان با دکمه ورود
...
function getValidNightTargets($role, $game, $playerId) {
    $targets = [];
    foreach ($game['players'] as $p) {
        if (($p['alive'] ?? false) && $p['id'] != $playerId) {
...
// ============================================================
// 6. پردازش شب و روز (بدون حلقه بی‌پایان)
// ============================================================
...
function processNight($game_code, $game) {
    ...
    // اگر کسی اکشن نداده باشد، اسکیپ محسوب می‌شود
...
function checkGameTimers() {
    ...
    foreach ($games as $code => $game) {
        ...
        if ($game['phase'] == 'night' && $now >= $game['night_end_time']) {
            // اگر اکشنی هست پردازش کن، وگرنه اسکیپ
            if (empty($game['night_actions'])) {
                // برای شب‌کارهایی که انتخاب نکردند، اسکیپ ثبت کن
                foreach ($game['players'] as $p) {
...
function sendPlayerList($chat_id, $game) {
    ...
    if ($game['status'] == 'waiting') {
        ...
        // دکمه ورود به روستا در پایین لیست
        $keyboard = ['inline_keyboard' => [
        ...
        foreach ($games as $code => $g) {
            foreach ($g['players'] as $p) {
                if ($p['id'] == $user_id && $g['status'] == 'started' && $g['phase'] == 'vote') {
                    ...
                    // اصلاح ارور Undefined variable $target_name
...
// پشتیبانی از دستورات با نام ربات (مثلاً /extend@Ni_cop_bot)
$parts = explode(' ', $text);
